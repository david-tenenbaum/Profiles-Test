   ...
   CREATE OR REPLACE VIEW MATERIAL_GONG_CALL_06C7C96D_32 AS 
SELECT
    *
FROM
...
WHERE
        ...
        ((
         DATE <= '2023-09-08T18:17:06.227495Z'
        )
         OR DATE IS NULL )
...
/* endMacros */
   ...
   CREATE OR REPLACE TEMPORARY TABLE MATERIAL_ACCOUNT_ID_STITCHER_6B1CE8F1_32_INTERNAL_EDGES_RAW 
        (
            id1 varchar,
            id1_type varchar,
            id2 varchar,
    ...
    ;  
                        ...
                        INSERT INTO MATERIAL_ACCOUNT_ID_STITCHER_6B1CE8F1_32_INTERNAL_EDGES_RAW (id1, id1_type, id2, id2_type, valid_at)
                            ...
                            FROM MATERIAL_SF_OPPORTUNITY_C340C2A8_32
                        ...
                        INSERT INTO MATERIAL_ACCOUNT_ID_STITCHER_6B1CE8F1_32_INTERNAL_EDGES_RAW (id1, id1_type, id2, id2_type, valid_at)
                            ...
                            SELECT
                                ...
                                'salesforce_id' AS id2_type,
                                MIN(CREATED_DATE)::timestamp AS valid_at
                            FROM MATERIAL_SF_ACCOUNT_84603234_32
                        ...
                        INSERT INTO MATERIAL_ACCOUNT_ID_STITCHER_6B1CE8F1_32_INTERNAL_EDGES_RAW (id1, id1_type, id2, id2_type, valid_at)
                            ...
                            FROM MATERIAL_GONG_CALL_06C7C96D_32
    ...
    DELETE FROM MATERIAL_ACCOUNT_ID_STITCHER_6B1CE8F1_32_INTERNAL_EDGES_RAW WHERE
        id1_type NOT IN (
    'salesforce_id'
)
        OR id2_type NOT IN (
    'salesforce_id'
                ...
                DELETE FROM MATERIAL_ACCOUNT_ID_STITCHER_6B1CE8F1_32_INTERNAL_EDGES_RAW WHERE NOT (
   ...
   CREATE OR REPLACE TABLE MATERIAL_ACCOUNT_ID_STITCHER_6B1CE8F1_32_INTERNAL_EDGES AS 
            SELECT
                id1,
                id1_type,
                id2,
...
,
    ...
    'rid' || left(sha1(concat(left(sha1('fe971b24-9572-4005-b22f-351e9c09274d' || NVL(id2,'')),32),NVL(id2_type,''))),32)
            ...
            FROM MATERIAL_ACCOUNT_ID_STITCHER_6B1CE8F1_32_INTERNAL_EDGES_RAW
            WHERE
                id1 IS NOT NULL
                AND id1_type IS NOT NULL
                AND id2 IS NOT NULL
        ...
        DROP TABLE MATERIAL_ACCOUNT_ID_STITCHER_6B1CE8F1_32_INTERNAL_EDGES_RAW;

        
   CREATE OR REPLACE TEMPORARY TABLE MATERIAL_ACCOUNT_ID_STITCHER_6B1CE8F1_32_INTERNAL_ACTIVE_MAPPING 
    (
        node_id varchar,
...
;  
   ...
   CREATE OR REPLACE TEMPORARY TABLE MATERIAL_ACCOUNT_ID_STITCHER_6B1CE8F1_32_INTERNAL_FINISHED_MAPPING 
    (
        node_id varchar,
        node_id_type varchar,
        account_main_id varchar,
...
;  
        ...
        INSERT INTO MATERIAL_ACCOUNT_ID_STITCHER_6B1CE8F1_32_INTERNAL_ACTIVE_MAPPING (
            node_id,
            node_id_type,
            account_main_id,
            valid_at,
        ...
        FROM
            MATERIAL_ACCOUNT_ID_STITCHER_6B1CE8F1_32_INTERNAL_EDGES
        UNION ALL
        SELECT
            id2 AS node_id,
   ...
   CREATE OR REPLACE TEMPORARY TABLE MATERIAL_ACCOUNT_ID_STITCHER_6B1CE8F1_32_INTERNAL_ACTIVE_MAPPING AS (
    SELECT
        node_id,
        node_id_type,
        account_main_id,
    ...
    FROM (
        ...
        FROM MATERIAL_ACCOUNT_ID_STITCHER_6B1CE8F1_32_INTERNAL_ACTIVE_MAPPING
    )
    WHERE row_number = 1
);  
   ...
   CREATE OR REPLACE TEMPORARY TABLE MATERIAL_ACCOUNT_ID_STITCHER_6B1CE8F1_32_INTERNAL_ACTIVE_MAPPING AS (
    ...
    INSERT INTO MATERIAL_ACCOUNT_ID_STITCHER_6B1CE8F1_32_INTERNAL_FINISHED_MAPPING (
    ...
    DELETE FROM MATERIAL_ACCOUNT_ID_STITCHER_6B1CE8F1_32_INTERNAL_ACTIVE_MAPPING WHERE stitching_active = 0;
   ...
   CREATE OR REPLACE TEMPORARY TABLE MATERIAL_ACCOUNT_ID_STITCHER_6B1CE8F1_32_INTERNAL_ACTIVE_MAPPING AS (
    ...
    FROM (
        ...
        FROM (
            ...
            FROM
                MATERIAL_ACCOUNT_ID_STITCHER_6B1CE8F1_32_INTERNAL_ACTIVE_MAPPING
        )
    )
    );  
   ...
   SELECT not_converged INTO :is_not_converged_var FROM (SELECT COUNT(*) > 0 AS not_converged FROM MATERIAL_ACCOUNT_ID_STITCHER_6B1CE8F1_32_INTERNAL_ACTIVE_MAPPING);  
 END WHILE;  

        
   CREATE OR REPLACE TABLE MATERIAL_ACCOUNT_ID_STITCHER_6B1CE8F1_32_INTERNAL_MAPPING AS SELECT node_id, node_id_type, account_main_id, valid_at, account_main_id_dist, stitching_active FROM MATERIAL_ACCOUNT_ID_STITCHER_6B1CE8F1_32_INTERNAL_FINISHED_MAPPING UNION ALL SELECT node_id, node_id_type, account_main_id, valid_at, account_main_id_dist, stitching_active FROM MATERIAL_ACCOUNT_ID_STITCHER_6B1CE8F1_32_INTERNAL_ACTIVE_MAPPING;  

        
   CREATE OR REPLACE VIEW MATERIAL_ACCOUNT_ID_STITCHER_6B1CE8F1_32 AS 
    WITH ranked_mappings AS (
        ...
        FROM MATERIAL_ACCOUNT_ID_STITCHER_6B1CE8F1_32_INTERNAL_MAPPING
    )
    SELECT
        account_main_id,
        other_id,
...
;  
 ...
 END;
   ...
   DROP VIEW IF EXISTS MATERIAL_GONG_CALL_06C7C96D_32;  
   ...
   DROP VIEW IF EXISTS MATERIAL_SF_OPPORTUNITY_C340C2A8_32;  
   ...
   DROP VIEW IF EXISTS MATERIAL_SF_ACCOUNT_84603234_32;  
